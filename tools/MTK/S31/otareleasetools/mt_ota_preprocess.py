...
def main(argv):
  ...
  for filename in partition_map:
    # if image with sig appened exist, use these verified image to update
    verified_filename = os.path.splitext(filename)[0] + "-verified" + os.path.splitext(filename)[1]
    src_filename = "%s/%s" % (out_folder, verified_filename)
    if not os.path.isfile(src_filename):
      src_filename = "%s/%s" % (out_folder, filename)
    dst_filename = "%s/IMAGES/%s" % (zip_root_folder, filename)
    dst_dir = os.path.dirname(dst_filename)
    if not os.path.exists(dst_dir):
      os.makedirs(dst_dir)
    print "[OTA Preprocess] Copy from %s to %s" % (src_filename, dst_filename)
    try:
      shutil.copy(src_filename, dst_filename)
    except:
      print "[OTA Preprocess] image '%s' is not exist" % src_filename
